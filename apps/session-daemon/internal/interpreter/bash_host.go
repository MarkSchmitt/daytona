...
func NewBashFactory(cfg *config.Config, logger *slog.Logger) (*BashFactory, error) {
	...
	scriptPath := filepath.Join(dir, "daytona_session_repl_bash_host.js")
	if err := os.WriteFile(scriptPath, []byte(bashHostScript), workerScriptPerms); err != nil {
		return nil, fmt.Errorf("write bash host script: %w", err)
	}
	return &BashFactory{
		cfg:         cfg,
