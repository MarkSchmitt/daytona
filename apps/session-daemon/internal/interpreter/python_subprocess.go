...
func NewPythonFactory(cfg *config.Config, logger *slog.Logger) (*PythonFactory, error) {
	dir := cfg.WorkerScriptDir
	if dir == "" {
		dir = os.TempDir()
	}
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return nil, fmt.Errorf("create worker script dir: %w", err)
	}
	path := filepath.Join(dir, "daytona_session_repl_worker.py")
	if err := os.WriteFile(path, []byte(pythonWorkerScript), workerScriptPerms); err != nil {
